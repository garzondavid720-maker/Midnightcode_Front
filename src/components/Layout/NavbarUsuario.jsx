...
const NavbarUsuario = ({ onCartToggle, cartCount = 0 }) => {
  ...
  const notificaciones = [
    { id: 1, mensaje: "Nueva canción añadida a la playlist", tiempo: "hace 5 min" },
    { id: 2, mensaje: "Tu reserva para el VIP está confirmada", tiempo: "hace 2 h" },
    { id: 3, mensaje: "Evento 'Noche de electrónica' comienza en 1 día", tiempo: "hace 6 h" },
    { id: 4, mensaje: "Actualización de seguridad disponible", tiempo: "hace 1 día" },
  ];
        ...
        return (
          <div className="p-4 space-y-3">
            <div className="flex items-center gap-3 pb-3 border-b border-white/10">
              ...
              <div>
                <p className="font-label-md text-on-surface font-bold">{usuarioNombre}</p>
                <p className="text-xs text-on-surface-variant">Usuario</p>
              </div>
            </div>
    ...
    <>
      <nav className="fixed top-0 w-full z-50 bg-surface/70 backdrop-blur-xl border-b border-white/10 shadow-[0_0_15px_rgba(233,179,255,0.1)] flex justify-between items-center px-margin-mobile md:px-margin-desktop py-4 max-w-full">
        ...
        <div className="hidden md:flex items-center gap-6">
          ...
          <a
            className={`font-label-md text-label-md transition-all duration-300 hover:bg-white/5 px-2 py-1 rounded ${
              isActive("/usuario/cancion")
                ? "text-primary border-b-2 border-primary pb-1"
                : "text-on-surface-variant hover:text-primary"
            }`}
            href="/usuario/cancion"
          >
            Canciones
          </a>
          <a
            className={`font-label-md text-label-md transition-all duration-300 hover:bg-white/5 px-2 py-1 rounded ${
              isActive("/usuario/reserva")
                ? "text-primary border-b-2 border-primary pb-1"
                : "text-on-surface-variant hover:text-primary"
            }`}
            href="/usuario/reserva"
          >
            Reserva
          </a>
          <a
            className={`font-label-md text-label-md transition-all duration-300 hover:bg-white/5 px-2 py-1 rounded ${
              isActive("/usuario/menu")
                ? "text-primary border-b-2 border-primary pb-1"
                : "text-on-surface-variant hover:text-primary"
            }`}
            href="/usuario/menu"
        ...
        {/* Acciones: Carrito y Avatar */}
        <div className="flex items-center gap-4">
          {/* Botón Carrito */}
          <button
            onClick={onCartToggle}
            className="relative p-1.5 rounded-full hover:bg-white/5 transition-colors active:scale-95"
          ...
          >
            ...
            {cartCount > 0 && (
              <span className="absolute -top-0.5 -right-0.5 bg-secondary text-on-secondary text-[10px] font-bold w-5 h-5 rounded-full flex items-center justify-center shadow-[0_0_8px_rgba(255,178,183,0.6)]">
      ...
      <style jsx>{`
        ...
        .shadow-\\[0_0_8px_rgba\\(255\\,178\\,183\\,0\\.6\\)\\] {
          box-shadow: 0 0 8px rgba(255, 178, 183, 0.6);
        }
        .px-margin-mobile {
          padding-left: 16px;
          padding-right: 16px;
