...
export default function MenuPreview() {
  ...
  // Función para descargar el menú como .txt
  const handleDownload = () => {
    let content = "=== MENÚ LIQUIDOS LUXURY ===\n\n";
    content += "--- CÓCTELES DE AUTOR ---\n";
    COCKTAILS.forEach((item) => {
      content += `${item.name}\n`;
      content += `  ${item.desc}\n`;
      content += `  ${formatCOP(item.price)}\n\n`;
    });
    content += "--- BOTELLAS DE PRESTIGIO ---\n";
    ...
    link.href = url;
    link.download = "menu_liquidos_luxury.txt";
  ...
  return (
    <section
      className="py-24 px-6 relative bg-background-dark/80 overflow-hidden"
      id="menu"
    >
      <div className="max-w-7xl mx-auto relative z-10" ref={ref}>
        ...
        <div className="mt-16 text-center">
          <button
            ...
            className="border border-primary text-primary px-8 py-3 rounded-full font-bold uppercase tracking-widest hover:bg-primary hover:text-white transition-all"
          >
